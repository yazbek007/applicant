...
"""
إضافة باكتيست متكامل لاختبار أداء الإشارات على بيانات تاريخية
نسخة محسّنة: تتعامل مع الأخطاء بشكل منفصل لكل عملة وتستمر في التنفيذ
"""
...
# ======================
# إعدادات الباكتيست (يمكنك تعديلها)
# ======================
class BacktestConfig:
    MONTHS = 1                      # عدد الأشهر للاختبار
    TIMEFRAME = '15m'               # نفس الإطار المستخدم في الكشف
    ...
    STOP_LOSS_PCT = 1.0              # وقف الخسارة (1% عكس الاتجاه)
...
class Backtester:
    def __init__(self, detector):
        ...
        self.results = {}            # تخزين النتائج النهائية
        self.running = False
    ...
    def fetch_historical_data(self, symbol: str, months: int, timeframe: str) -> Optional[List]:
        ...
        تم إصلاح مشكلة حساب since باستخدام الوقت الحالي بالميلي ثانية
        """
        all_candles = []
        
        # ✅ الطريقة الصحيحة لحساب وقت البدء: ناقص 6 أشهر من الآن
        now_ms = int(time.time() * 1000)
        since = now_ms - (months * 30 * 24 * 60 * 60 * 1000)  # تحويل الأشهر إلى ميلي ثانية
        
        limit = 1000  # الحد الأقصى لكل طلب في Binance

        # تجهيز رمز الزوج بشكل صحيح (إزالة الشرطة المائلة)
        symbol_clean = symbol.replace('/', '') if '/' in symbol else symbol

        while True:
            try:
                candles = self.binance.exchange.fetch_ohlcv(
                    symbol_clean, timeframe, since=since, limit=limit
                )
                if not candles:
                    break
                ...
                if len(candles) < limit:
                    break
                # تحديث since إلى وقت آخر شمعة + 1 ميلي ثانية
                since = candles[-1][0] + 1
    ...
    def run_backtest_for_coin(self, coin) -> Dict:
        """
        تشغيل الباكتيست لعملة واحدة مع معالجة الأخطاء وإرجاع النتائج حتى لو فشلت
        """
        ...
        try:
            ...
            if not candles or len(candles) < 100:
                return {"error": f"Insufficient data: got {len(candles) if candles else 0} candles"}
            ...
            for i in range(min_bars_needed, total_candles):
                ...
                try:
                    ...
                    logger.debug(f"Indicator calculation error at index {i}: {e}")
                    continue  # تخطي هذه الشمعة إذا فشلت المؤشرات
                ...
                # Multi-timeframe confirmation (غير متوفرة في الباك تست، نمرر قيماً افتراضية)
            ...
            for sig in signals:
                ...
                for j in range(idx + 1, end_idx + 1):
                    ...
                    if signal_type == 'TOP':
                        ...
                        if price_change_pct >= BacktestConfig.PROFIT_TARGET_PCT:
                            success = True
                            exit_price = current_price
                            exit_reason = "target_hit"
                            break
                        elif (current_price - entry_price) / entry_price * 100 > BacktestConfig.STOP_LOSS_PCT:
                            success = False
                            exit_price = current_price
                        ...
                        if price_change_pct >= BacktestConfig.PROFIT_TARGET_PCT:
                            ...
                            exit_reason = "target_hit"
                            break
                        elif (entry_price - current_price) / entry_price * 100 > BacktestConfig.STOP_LOSS_PCT:
                            success = False
                            exit_price = current_price
                            exit_reason = "stop_loss"
                            break
            ...
            logger.exception(f"❌ Critical error in backtest for {coin.symbol}: {e}")
            return {"error": f"Critical error: {str(e)}"}

    def run_all(self) -> Dict:
        """
        تشغيل الباكتيست على جميع العملات مع الاستمرار حتى لو فشلت بعضها.
        يتم تخزين النتائج لكل عملة بشكل منفصل وإرسال التقرير في النهاية.
        """
        with self.lock:
            self.running = True
            all_results = {}
            ...
            for coin in AppConfig.COINS:
                if not coin.enabled:
                    continue
                try:
                    logger.info(f"🔄 ===== Starting backtest for {coin.symbol} =====")
                    res = self.run_backtest_for_coin(coin)
                    all_results[coin.symbol] = res
                    logger.info(f"✅ ===== Completed backtest for {coin.symbol} =====")
                except Exception as e:
                    logger.error(f"❌ Unhandled error for {coin.symbol}: {e}")
                    all_results[coin.symbol] = {"error": f"Unhandled: {str(e)}"}
            
            self.running = False
            self.results = all_results
            
            # إرسال التقرير بعد اكتمال جميع العملات (أو محاولة إرسال حتى لو بعضها فشل)
            self.send_report()
            
            return all_results

    def generate_report(self) -> str:
        """توليد تقرير نصي للنتائج بشكل مرتب"""
        if not self.results:
            return "❌ No backtest results available."

        lines = []
        lines.append("📊 **Backtest Results (Last {} Months)**".format(BacktestConfig.MONTHS))
        lines.append("")
        ...
        for symbol, res in self.results.items():
            ...
            if res.get('total_signals', 0) == 0:
                lines.append(f"⚠️ **{symbol}**: No signals generated")
                continue
        ...
        if coins_with_data > 0 and total_signals > 0:
            overall_win = total_success / total_signals * 100
            lines.append("**📈 Overall Summary**")
            lines.append(f"- Coins with data: {coins_with_data}")
            ...
            lines.append("No valid data to summarize.")

        return "\n".join(lines)

    def send_report(self):
        """إرسال التقرير إلى ntfy مع معالجة الأخطاء"""
        try:
            report = self.generate_report()
            title = f"Backtest Results - {BacktestConfig.MONTHS} Months"
            logger.info("📤 Sending backtest report to ntfy...")
            success = self.detector.notification_manager.send_ntfy(report, title, priority="4", tags="bar_chart")
            if success:
                logger.info("✅ Backtest report sent successfully.")
            else:
                logger.error("❌ Failed to send backtest report.")
        except Exception as e:
            logger.error(f"Error sending report: {e}")

# ======================
# Routes للتشغيل وعرض النتائج
# ======================

@app.route('/api/run_backtest', methods=['POST'])
def run_backtest():
    """تشغيل الباكتيست بشكل غير متزامن مع تحسينات"""
    if hasattr(app, 'backtester_running') and app.backtester_running:
        return jsonify({
            "status": "error", 
            "message": "Backtest already running. Please wait."
        }), 409
    ...
    def backtest_thread():
        ...
        try:
            ...
            # يمكنك أيضاً تخزين النتائج في ملف أو قاعدة بيانات إذا أردت
            logger.info(f"Backtest completed. Results: {list(results.keys())}")
        except Exception as e:
            logger.error(f"Backtest thread crashed: {e}")
        finally:
            app.backtester_running = False

    # التأكد من وجود المتغير
    if not hasattr(app, 'backtester_running'):
        app.backtester_running = False
    ...
    return jsonify({
        "status": "success",
        "message": "Backtest started. Results will be sent to ntfy shortly."
    })

@app.route('/api/backtest_results', methods=['GET'])
def get_backtest_results():
    """عرض آخر نتائج الباكتيست المخزنة"""
    backtester = Backtester(detector)
    return jsonify(backtester.results)

@app.route('/api/backtest_status', methods=['GET'])
def backtest_status():
    """معرفة ما إذا كان الباكتيست قيد التشغيل"""
    running = getattr(app, 'backtester_running', False)
    return jsonify({
        "running": running,
        "message": "Backtest is running" if running else "Backtest is idle"
    })
